...
const PriceStep: React.FC<PriceStepProps> = ({ price, setPrice }) => {
  // workspaceData não está sendo usado neste componente
  ...
  const handlePriceChange = useCallback((e: ChangeEvent<HTMLInputElement>) => {
    ...
    setPrice(number / 100);
  }, [setDisplayValue, setPrice]);

  const bodyContent = useMemo(
    () => (
      <div className="flex flex-col gap-8">
        <Heading title="Agora, defina seu preço" subtitle="Quanto você deseja cobrar por dia?" />
        <div className="w-full relative">
          ...
          <p className="text-sm text-gray-500 mt-2">Preço por dia de utilização do espaço</p>
        </div>
      </div>
    ),
    [displayValue, handlePriceChange]
  );
